...
def get_cpu_usage():
    """
    Return the percentage processor usage, as an int, of each processor for the machine running the script that called
    this function.

    :return: The processor usage percentages, as a list of integers.
    """

    return [int(round(cpu.PercentProcessorTime)) for cpu in wmi.WMI().Win32_PerfFormattedData_PerfOS_Processor()]


def get_ram_usage():
    """
    Return the percentage of RAM usage, as an int, for the machine running the script that called this function.
...
def get_io_latency(drive_letter='C:'):
    ...
    :param drive_letter: The drive letter to check.
    :return: The disk usage for the indicated drive, as a tuple (read_latency, write_latency, transfer_latency).
    """
    ...
    return only((latency.AvgDiskSecPerRead, latency.AvgDiskSecPerWrite, latency.AvgDiskSecPerTransfer)
